      ...
      try {
        ...
        if (response.success && Array.isArray(response.rides)) {
          const uniqueRides = Array.from(
            new Map(response.rides.map((ride) => [ride._id, ride])).values()
          );
        ...
        <Tabs defaultValue="upcoming" className="space-y-4">
          ...
          <TabsContent value="past" className="space-y-4">
            {loading ? (
              ...
              ridesByMe.map((ride) => {
                const rideDateTime = parseISO(`${ride.date}T${ride.time}`);

                const now = new Date();

                const expired = rideDateTime.getTime() < now.getTime();
                return (
                  <Card
                    key={ride._id}
